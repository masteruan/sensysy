...
void setup_wifi() {
  ...
  //change IP address
  IPAddress ipa(10, 13, 0, 15); //10.13.0.14 - 15 - 16
...
void loop() {
  ...
  if (now - lastMsg > 10000) {
    ...
    client.publish("lux2",String(lux).c_str(), true);
    client.publish("humi2",String(h).c_str(), true);
    client.publish("gas2",String(aria).c_str(), true);
    client.publish("amp2",String(AmpsRMS).c_str(), true);
    /*
    leds[0] = CRGB::Red;
    FastLED.show();
    delay(500);
